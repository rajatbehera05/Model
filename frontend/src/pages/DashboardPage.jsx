        ...
        {showReserveModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl border border-[#E2E8F0] text-[#0F172A]"
            >
              ...
              <div className="mt-6 flex justify-end">
                <button
                  onClick={() => setShowReserveModal(false)}
                  className="px-4 py-2 text-xs font-semibold text-[#64748B] hover:text-[#0F172A] hover:bg-slate-100 rounded-lg transition-colors"
                >
                  Cancel
                </button>
              </div>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-lg border flex items-center gap-2.5 text-xs font-semibold ${
              toast.type === 'success'
